...
export class CustomerStore {
  ...
  readonly customer = signal<CustomerState | null>(null);
  ...
  async refresh() {
    ...
    this.error.set('');
    try {
      const response = await this.auth.request('/api/customer');
      if (!response.ok) throw new Error();
      const result = customerStateResponseSchema.parse(await response.json());
      if (
      ...
      this.stale.set(true);
      this.error.set(
        'Saved customer settings are unavailable. Check your connection and access, then refresh.',
      );
    } finally {
