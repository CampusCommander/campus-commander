...
export class DiagnosticsStore implements OnDestroy {
  ...
  readonly errorContext = signal<'connections' | 'checks' | null>(null);
  ...
  readonly ageSeconds = computed(() => {
    ...
    return health ? Math.max(0, Math.floor((this.clock() - Date.parse(health.observedAt)) / 1000)) : 0;
  });
  ...
  async refresh() {
    ...
    try {
      ...
      this.errorContext.set('connections');
    } finally {
