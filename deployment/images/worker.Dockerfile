...
RUN npm exec -- nx run worker:build
RUN find /workspace/dist/worker /workspace/dist/deployment -exec touch -d "@${SOURCE_DATE_EPOCH}" {} +
...
USER node
EXPOSE 3001
HEALTHCHECK --interval=10s --timeout=3s --start-period=10s --retries=3 CMD ["node", "-e", "const fs=require('node:fs'),https=require('node:https');const tls=process.env.TLS_CERT_FILE;if(!tls){fetch('http://127.0.0.1:3001/health').then(r=>{if(!r.ok)process.exit(1)}).catch(()=>process.exit(1));}else{https.get({hostname:'127.0.0.1',port:3001,path:'/health',servername:process.env.TLS_SERVER_NAME,ca:fs.readFileSync(process.env.TLS_CA_FILE||tls)},r=>{r.resume();process.exit(r.statusCode===200?0:1)}).on('error',()=>process.exit(1));}"]
CMD ["node", "main.js"]
