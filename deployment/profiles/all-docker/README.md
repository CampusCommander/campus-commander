...
Run the installer on the Docker daemon host. A controller container with a mounted host Docker socket has a different filesystem. The installer checks daemon access with a temporary private marker before it prepares services. The check compares marker contents and removes the marker after the check. A filesystem mismatch fails the `docker-installation-filesystem` prerequisite. Source bind mounts also reject missing paths instead of creating directories.
