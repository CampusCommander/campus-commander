...
for (const phase of [2, 3])
  for (const profile of ['all-docker', 'hybrid', 'kubernetes']) {
    for (const provider of ['oidc', 'google']) {
      test(`guided ${profile} Phase ${phase} configuration collects protected ${provider} credentials`, async () => {
        const seen = new Set();
        const q = async (key, _label, fallback) => {
          seen.add(key);
          if (key === 'phase') return String(phase);
        ...
        assert.equal(plan.config.phase, phase);
        assert.equal(
        ...
        assert.equal(
          JSON.stringify(plan.config).includes('/protected/'),
          false,
        );
        if (provider === 'google') {
