...
for (const profile of ['all-docker', 'hybrid', 'kubernetes']) {
  test(`guided ${profile} Phase 2 configuration collects protected OIDC credentials`, async () => {
    const seen = new Set();
    ...
    const source = `/protected/${profile === 'kubernetes' ? 'campus-oidc.client-secret' : 'oidc-client'}`;
