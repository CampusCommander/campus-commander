...
**S1. Force sign-out needs a scope we do not request.** `users.signOut` requires `admin.directory.user.security`, confirmed at https://developers.google.com/admin-sdk/directory/reference/rest/v1/users/signOut. The v1 bulk catalog includes force sign-out (Spec 1, Users). The wizard list has only `admin.directory.user`. Fix: add `admin.directory.user.security` to the DWD scope list in the wizard and `03-architecture.md`.
