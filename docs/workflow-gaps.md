...
| --- | --- | --- | --- |
| G01 | Add a person to Campus Commander — **disputed** | Internal RBAC in the [original spec](archive/specs/2026-07-07-core-entity-management-design.md#authentication--authorization). "Invite-only" in [older decision 17.7](archive/architecture/planning-review-topics.md#177-rbac-model). A detailed link/redemption/confirmation flow in the [implementation record](portfolio/phase-3-invitations.md). | How does an administrator identify and add someone: directory selection, email entry, first-sign-in approval, or another method? Is any invitation delivery needed? What does the new person do? The owner now disputes the built invitation system. Do not adopt one of these options by default. |
| G02 | Assign and edit permissions — **partial, scope disputed** | Two older roles, then action/resource grants and four presets in [R12](portfolio/05-decisions-and-open-questions.md#current-decisions). A technical action matrix in [authorization](portfolio/phase-3-authorization.md). | Which roles should administrators see? Are custom permissions required? Which resources can they select? Who can delegate, and how does the editor explain effective access? No complete product-specific Figma flow was found. |
...
| G05 | Navigation and landing page — **missing composition for setup/access** | Shared shell rules and Devices/Jobs/Settings designs. Phase 3 added separate connection, settings, account, Diagnostics, Schools, access, and invitation destinations. | What opens after sign-in? Which administration tasks belong under Settings? What remains visible before Google connection? No design source establishes the implemented seven-item administration menu. |
...
| G07 | Customer settings — **incomplete product scope** | The [implementation record](portfolio/phase-3-customer-settings.md#settings-contract) defines only a display name and says further settings can be identified later. | Which settings must the product expose? How are connection, access, sync configuration, and preferences grouped? Do administrators need visible revisions, setup history, or save receipts for a name change? |
...
| G23 | Audit history — **records defined, product surface partial** | [Audit architecture](portfolio/03-architecture.md#operations-audit-and-recovery), Jobs details, opportunity E4. | Decide whether a separate audit explorer belongs in scope, who sees it, its filters, and downloads. Backend audit records do not authorize another navigation page. |
...
The next discussion needs G01–G07. Later workflow questions stay attached to their own development tasks.
