...
main() {
  ...
  cc_stop_progress() {
    if [ -n "${cc_progress_pid:-}" ]; then
      kill "$cc_progress_pid" 2>/dev/null || :
      wait "$cc_progress_pid" 2>/dev/null || :
      cc_progress_pid=
    fi
  }
  cc_report_progress() {
    cc_timer_pid=
    trap '[ -z "$cc_timer_pid" ] || kill "$cc_timer_pid" 2>/dev/null || :; exit 0' TERM INT
    cc_elapsed=0 cc_quiet=0 cc_previous=0
    while :; do
      sleep 5 &
      cc_timer_pid=$!
      wait "$cc_timer_pid" || exit 0
      cc_timer_pid=
      cc_elapsed=$((cc_elapsed + 5))
      cc_bytes=$(wc -c < "$cc_system_log")
      if [ "$cc_bytes" -gt "$cc_previous" ]; then
        cc_quiet=0
        cc_status 'Log activity' "${cc_elapsed}s elapsed, $((cc_bytes - cc_previous)) new log bytes."
      else
        cc_quiet=$((cc_quiet + 5))
        cc_status 'Waiting' "${cc_elapsed}s elapsed. No new log output for ${cc_quiet}s."
        if [ "$cc_quiet" -ge 30 ]; then
          cc_say '  The task has not exited. Silence does not confirm progress. Inspect the detailed log if waiting continues.'
        fi
      fi
      cc_previous=$cc_bytes
    done
  }
  cc_run_system_step() (
    cc_system_label=$1
    ...
    cc_authorize
    cc_progress_pid=
    trap 'cc_stop_progress' EXIT
    trap 'exit 130' INT
    trap 'exit 143' TERM
    if command -v mktemp >/dev/null 2>&1 && command -v sleep >/dev/null 2>&1 && command -v wc >/dev/null 2>&1; then
      ...
      cc_say '  Activity updates appear every 5 seconds. Detailed package output remains in the private log.'
      cc_report_progress &
      cc_progress_pid=$!
      ...
      cc_stop_progress
      cc_status 'Completed' "$cc_system_label"
    else
